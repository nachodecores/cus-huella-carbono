...
export default async function SubmissionDetailPage({ params }: PageProps) {
    ...
    return (
      <div className="mx-auto max-w-lg px-4 py-16">
        ...
        <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
          No hay un envío con ese identificador para tu empresa.
        </p>
      </div>
    );
  }
  ...
  if (row.status === "draft") {
    redirect(`/e/${encodeURIComponent(token)}/s/${submissionId}/edit`);
  }
  ...
  return (
    <>
      ...
      />
      <p className="mx-auto max-w-2xl px-4 pb-10">
        <Link
          href={basePath}
          className="text-sm text-neutral-600 underline hover:text-neutral-900 dark:text-neutral-400"
        >
          ← Volver al listado
        </Link>
      </p>
